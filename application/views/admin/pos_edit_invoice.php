 ...
 $rand_keys = array_rand($color, 2);
  ...
  <div class="container white">
    <div class="row ">
      ...
      <h2><u>Print Reciept</u></h2>
    </div>
      <div class="col-md-6" style="margin-top: 25px;">
      <a href="<?php echo base_url('admin/sales/sale_history'); ?>"><button style="float: right;" class="btn btn-success">Go Back</button></a>
    </div>    
      ...
      <div class="cart_class">
        ...
        <div class="col-sm-8">
               <a href='<?php echo base_url().'pos/'?>getPrint_edit.php' data-toggle="modal" data-target="#myModal" data-remote="false" ><button onclick="printnowme('print_it')" id="printnowme" class="btn btn-default" id="sb" > 
            Print Now</button></a>
                  </div>
      </div>
      </div>
...
//          $("#added_"+product_code).show();
 ...
 cartActionEdit = function(action,product_code,barcode,quantity,pro_id,discount,segment,edit_invoice) {
var id_inv = "<?php echo $editItem[0]->id_inv; ?>";
edit_invoice = "<?php echo $this->uri->segment(3);?>";
...
var myDisc =$('#discount_O').val();
  ...
  if(action != "") {
    ...
    switch(action) {
      case "add":
        queryString = 'action='+action+'&code='+ product_code+'&quantity='+quantity+"&barcode="+barcode+"&pro_id="+pro_id+"&discount="+myDisc+"&segment="+segment+'&id_inv='+ id_inv+'&edit_invoice='+ edit_invoice;
      break;
      case "remove":
  ...
  jQuery.ajax({
  url: "<?php echo base_url().'pos/'?>ajax_action_print.php",
  data:queryString,
  type: "POST",
  success:function(data){
...
window.onload=function(){
  ...
  $(document).ready(function(){
    ...
    $('#sb').click(function(){ 
      var url = "<?php echo base_url().'admin/sales/sale_history'; ?>";   
      var Query = $('#Query').val();    
...
function closer(em)
    ...
    function printnowme(em)
    {
      document.getElementById("printnowme").style.display = "none";  //hide
  ...
  <div class="modal-dialog">
    <div class="modal-content super-content">
      ...
      <div class="modal-footer">
        ...
        <button type="button" onclick="closer('closeme')" class="btn btn-default" data-dismiss="modal">Close</button>
      </div>
    </div>
  </div>
</div>
...
?>
 ...
 <script type="">
  ...
  cartActionEdit('add','<?php echo $value->pro_id; ?>','<?php echo $value->barcode; ?>','<?php echo $value->quantity; ?>','<?php echo $value->pro_id; ?>','<?php echo $value->discount; ?>','<?php echo $this->uri->segment(4); ?>','edit_invoice');
