		...
		<div id="page-wrapper">
			<div class="main-page">
				<div class="forms">
					...
					<h2 class="title1">Expense Category  </h2>
					</div>

				<div class="form-three widget-shadow">
					
							<form action="<?= base_url().$path.'/category/insert_exp_category';?>" method="post" class="form-horizontal">
<?php if($msg){
						?>
					<div class="alert alert-success">
						...
						<?php	
  						...
  						<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>									    
							  ...
							  <?php } ?>									
								...
								<div class="form-group">
									<label for="focusedinput" class="col-sm-2 control-label">Category Detail</label>
									<div class="col-sm-8">
										<textarea rows="6" cols="79" required="" name="category_detail" ></textarea>
									</div>
						...
						<table class="table table-bordered" id="employee_id">
							...
							<?php  $sr =1; foreach($categoryExp as $ser): ?>
							<tr>
								<td><?= $sr++; ?></td>					
								<td><?= $ser->category; ?></td>

								<?php if($userPermission->can_edit == 'yes'){  ?> 
								<td><a href="<?= base_url().$path.'/category/edit_exp_category/'.$ser->exp_cate_id; ?>"><button class="btn btn-info">Edit</button></a></td>
<?php } else { ?>
<td><button onclick="failedRec();" class="btn btn-info">Edit</button></td>
<?php } if($userPermission->can_delete == 'yes'){  ?>
								<td><a onclick="if(confirm('Do you want to delete this Expense?')){ return true; } else {return false;} " href="<?= base_url().$path."/category/delete_exp_category/".$ser->exp_cate_id?>"><button class="btn btn-danger">Delete</button></a></td>																																
                     <?php } else{ ?> 
