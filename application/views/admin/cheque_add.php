...
.sidebar-left{height: 700px;}
}
@media only screen and (min-width: 991px) {
.sidebar-left{height: 800px;}
}
@media only screen and (min-width: 991px) {
.sidebar-left{height: 850px;}
}

@media only screen and (min-width: 1081px) {
.sidebar-left{height: 900px;}
}
</style>
			...
			<div class="main-page">
				<div class="forms">
					...
					<div class="row">
						...
						<h2 class="title1">Add Cheque</h2>
				</div>
					<?php $msg = $this->session->flashdata('msg'); ?>
					<?php $remove = $this->session->flashdata('remove');  ?>				
				<div class="form-three widget-shadow">
					
							<form action="<?= base_url().$path.'/settings/insertCheque';?>" method="post" class="form-horizontal" enctype="multipart/form-data">
<?php if($msg){	?>
 ...
 ?>									
								...
								<div class="form-group">
									<label for="focusedinput" class="col-sm-2 control-label">Cheque Book</label>
									<div class="col-sm-8">
										<input type="text" required="" name="cheque_book" value="" class="form-control1">
								...
								<div class="form-group">
									<label for="focusedinput" class="col-sm-2 control-label">Leaf </label>
									<div class="col-sm-8">
										<input type="text" value="" required="" name="leaf" class="form-control1">
									</div>
...
<?php if($userPermission->can_edit == 'yes'){  ?>
								<td><a href="<?= base_url().$path.'/banking/edit_cheque/'.$b->cheque_id; ?>"><button class="btn btn-info">Edit</button></a></td>
								<?php } else{  ?>
								<td><button onclick="failedRec()" class="btn btn-info">Edit</button></td>
<?php } if($userPermission->can_delete == 'yes'){  ?>																
								<td><a onclick="if(confirm('Do you want to delete this category?')){ return true; } else {return false;} " href="<?= base_url().$path."/banking/delete_cheque/".$b->cheque_id?>"><button class="btn btn-danger">Delete</button></a></td>
								<?php } else { ?>
									<td>
									<button onclick="failedRec()" class="btn btn-danger">Delete</button>
									</td>									
								<?php } ?>																																
							</tr>
						<?php endforeach; ?>
