<?php $user = $this->session->userdata('userVal'); 
      ...
      $editItem  = $this->sc_model->get_sale_edit($id); 
//print_r($editItem);
if(!empty($user)){
      ...
      <div class="col-md-4">
        <div class="row ">
          ...
          <div class="col-md-8 anchor">
            ...
            <a class="" href="<?php echo base_url('logout'); ?>"><i class="fa fa-sign-out"></i>  Log Out</a>
          </div>
        </div>
...
<div class="navbar">
  ...
  <button class="btn btn-primary" data-toggle="modal" data-target="#myModals" style="margin-top: 10px;margin-right: 15px;width: 110px;margin-left: 5px;">Calculator</button>
  <a href="<?php echo base_url()?>admin/dashboard"><button class="btn btn-success" style="float:left;margin-right: 1px;margin-top: 10px;width: 110px;">Home</button></a>
  ...
  <div class="container white">
    <div class="row ">
     ...
     if($editItem[0]->customer_name == $c['firstname']." ".$c['lastname']  ){ ?>
    <option selected="" value="<?php echo $editItem[0]->customer_id; ?>"><?php echo $editItem[0]->customer_name; ?></option>
  <?php } else{ ;?>
    ...
    <select id="tax_info" style="margin-bottom: 10px;margin-top:6px;width: 500px;" class="form-control" >
  <option value="">Select Tax</option>
  <option value="">No Tax</option>
<?php     
    $tax = $this->sc_model->getTax();
        foreach($tax as $ti){  ?>
        <option value="<?php echo $ti['tax_percentage'];?>"><?php echo $ti['tax_name']." ".$ti['tax_percentage'].$ti['symbol'];?></option>
...
$(document).ready(function(){
 ...
 if(isNaN(id)){ 
  id ="";
    ...
    $.ajax({
      url: '<?php echo base_url()."pos/ajax_action_edit.php";?>',
      type: 'post',
      data: {cut:id},
...
//          $("#added_"+product_code).show();
 ...
 cartActionEdit = function(action,product_code,barcode,quantity,pro_id,discount,segment) {
var id_inv = "<?php echo $editItem[0]->id_inv; ?>";
var a =10
if(a == 10){
  var checQuantity =   $('body').find('input[id="pro_stock' + pro_id + '"]').val();
var myDisc =$('#discount_O').val();
  ...
  if(action != "") {
    ...
    switch(action) {
      case "add":
        queryString = 'action='+action+'&code='+ product_code+'&quantity='+quantity+"&barcode="+barcode+"&pro_id="+pro_id+"&discount="+myDisc+"&segment="+segment+'&id_inv='+ id_inv;
      break;
      case "remove":
        queryString = 'action='+action+'&barcode='+ barcode+'&id_inv='+ id_inv;
      break;
      case "empty":
...
function cartActionQ(action,barcode,myquantity,product_code,pro_id,discount) {
  var bCode = String(barcode)
  //var pCode = String(pro_id)
  if(myquantity ==''){
...
swal(
  ...
  if(action != "") {
    switch(action) {
      case "add":
        queryString = 'action='+action+'&code='+ product_code+'&quantity='+myquantity+'&barcode='+barcode+'&pro_id='+pro_id+'&discount='+discount;
      break;
      case "remove":
        queryString = 'action='+action+'&code='+ product_code;
      break;
...
$("#filter").on("keyup", function() {
 ...
 var action  = 'findMe' 
  url ='<?php echo base_url().'pos/'?>products.php';
  var currenUrl ="<?php echo base_url(uri_string()); ?>";
$.ajax({
  type: 'POST',
  url: url,
  data: {id:id,keyword:keyword,action:action,url:currenUrl},
  beforeSend: function(){
...
$('#getCategoryItem').change(function(){
  ...
  id = $('#getCategoryItem').val();
  url ='<?php echo base_url().'pos/'?>products.php';
  var currenUrl ="<?php echo base_url(uri_string()); ?>"; 
$.ajax({
  type: 'POST',
  url: url,
  data: {id:id,url:currenUrl},
  success: function(data) {
...
window.onload=function(){
  ...
  var url = "<?php echo base_url().'admin/sales/add_sale_invoice_edit'; ?>";   
      var Query = $('#Query').val();    

      var QuantityList = $('#getQuantityList').val();
      var proID = $('#getproductIDList').val();
      var productIDQuery = $('#productIDQuery').val();
      var fullCartID = $('#fullCart').val();
      var updateInsert = $('#updateInsert').val();      
      ...
      var quantityCheck = $('#quantityCheck').val();
  ...
  $.ajax({
    type: "POST",
    url: url,
    data:{PayM:PayM,paid:paid,pro_id:pro_id,quantity:quantity,customer:customer,total:total,discount:discount,product_names:proNames,customerName:customerName,Query:Query,proID:proID,QuantityList:QuantityList,proQuery:productIDQuery,fullCartID:fullCartID,updateInsert:updateInsert,invoice_id:invoice_id,quantityCheck:quantityCheck},
    beforeSend: function(){
      $("#search-box").css("background","#FFF url(LoaderIcon.gif) no-repeat 165px");
    },
    success: function(data)
...
autoCartLoad = function(){
  var c_action ='empty';
  var id_inv = '<?php echo $editItem[0]->id_inv; ?>'
  var url1 = '<?php echo base_url()."pos/images/shopping.jpg";?>';
  url ='<?php echo base_url().'pos/'?>autoCat_edit.php';
$.ajax({
  type: 'POST',
  url: url,
  data: {C_action:c_action,id_inv:id_inv,url:url1},
  success: function(data) {
// var dropMe = 'delete'
    ...
    <div class="modal-content" style="height: 400px;width: 280px;">
      ...
      <div class="modal-body">
        ...
        <div class="form-three widget-shadow">
                ...
                <div class="form-group">
                  <div class="col-sm-8">
               <a href='<?php echo base_url().'pos/'?>getPrint_edit.php' data-toggle="modal" data-target="#myModal" data-remote="false" ><button class="btn btn-info" id="sb" style="margin-right:100px;margin-top:10px"> 
            Submit</button></a>
                  </div>
...
?>
 <input type="hidden"  id="discount_O" value="<?php echo $value->discount;?>">
 <script type="">
 
  cartActionEdit('add','<?php echo $value->pro_id; ?>','<?php echo $value->barcode; ?>','<?php echo $value->quantity; ?>','<?php echo $value->pro_id; ?>','<?php echo $value->discount; ?>','<?php echo $this->uri->segment(4); ?>');
